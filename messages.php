<?php session_start(); include_once "top_navi.php"; ?>
...
$result=p g_query($dbconn, $query); 
$_var=array(); 
while ($row=p g_fetch_row($result)) { 
    $_tarr=array('userName'=> $row[1], 'body' => $row[2], 'createdBy' => $row[1], 'sentOn' => $row[3]);                   array_push($_var, $_tarr); } pg_close($dbconn); 
    echo "<p> $_var </p>"; 
?>
        ...
        <div class="container">
            ...
            <div class="row" ng-controller="inboxCtrl">
                <div class="col-md-10">
                        ...
                        <div class="table-responsive" ng-show="!isMessageSelected()">
                            <table class="table table-striped table-hover refresh-container pull-down">
                                <thead class="hidden-xs">
                                    <tr>
                                        <td class="col-sm-3"><a href="javascript:;"><strong>Date / Time</strong></a>
                                        </td>
                                        <td class="col-sm-2"><a href="javascript:;"><strong>Customer</strong></a>
                                        </td>
                                        <td class="col-sm-4"><a href="javascript:;"><strong>Message</strong></a>
                                        </td>
                                ...
                                <tbody>
                                    <tr ng-repeat="item in pagedItems[currentPage] | orderBy:sortingOrder:reverse">
                                        ...
                                        <td class="col-sm-3 col-xs-4" ng-click="readMessage($index)"><span ng-class="{strong: !item.read}">{{item.sentOn | date:'yyyy-MM-dd hh:mm'}}</span>
                                        </td>
                                        <td class="col-sm-2 col-xs-4" ng-click="readMessage($index)"><span ng-class="{strong: !item.read}">{{item.userName}}</span>
                                        </td>
                                        <td class="col-sm-4 col-xs-6" ng-click="readMessage($index)"><span ng-class="{strong: !item.read}">{{item.body}}</span>
                                        </td>
                                        <td class="col-sm-1 col-sm-2" ng-click="readMessage($index)"><span ng-show="item.attachment" class="glyphicon glyphicon-paperclip pull-right"></span> <span ng-show="item.priority==1" class="pull-right glyphicon glyphicon-warning-sign text-danger"></span>
                                        </td>
                                    </tr>
                                </tbody>
                            </table>
                    ...
                    <!--/inbox panel-->
                    <div class="well well-s text-right"><em>Last updated: <span id="lastUpdated">{{date | date:'MM-dd-yyyy HH:mm:ss'}}</span></em>
                    </div>
                    ...
                    <div class="modal-dialog">
                        <div class="modal-content">
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="button" class="btn btn-primary ">Send <i class="fa fa-arrow-circle-right fa-lg"></i>
                                </button>
                            </div>
                        </div>
                        <!-- /.modal-content -->
